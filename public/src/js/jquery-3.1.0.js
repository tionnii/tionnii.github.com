...
function ipCheckOff() {
	...
	if (label != null) {
		...
		$('ckb_type').title='IP 蹂댁븞�� 爰쇱졇 �덉뒿�덈떎. IP蹂댁븞�� �ъ슜�섎젮硫� �좏깮�댁＜�몄슂.';
	}
	setSmartLevel(-1);
	var scriptElement = document.createElement('script');
...
function ipCheckOn(level) {
	...
	$('ckb_type').title='IP 蹂댁븞�� 耳쒖졇 �덉뒿�덈떎. IP蹂댁븞�� �ъ슜�섏� �딆쑝�쒕젮硫� �좏깮�� �댁젣�댁＜�몄슂.';
	setSmartLevel(level);
	var scriptElement = document.createElement('script');
...
function checkAndReloadKey(api_name) {
	if (keystr == null || keystr == undefined || keystr == "") {
		...
		if (api_name == "key_json") {
			scriptElement.setAttribute('src', 'http://static.nid.naver.com/enclogin/keys_json_legacy.nhn?callback=keySetting');
		} else {
			scriptElement.setAttribute('src', 'http://static.nid.naver.com/loginv3/js/keys_js_legacy.nhn');
		}
		document.getElementsByTagName('head')[0].appendChild(scriptElement);

		scriptElement = document.createElement('script');
		scriptElement.setAttribute('src', 'http://static.nid.naver.com/enclogin/fail_log.nhn?api=' + api_name + '&ua=' + encodeURIComponent(navigator.userAgent));
		document.getElementsByTagName('head')[0].appendChild(scriptElement);
	}
}
...
var loginNClicks = {
	...
	anchorTextNClicksNamePair : [
		[ "IP蹂댁븞", "log_off.ipcheck" ],
		[ "�뚯썝媛���", "log_off.registration" ],
		[ "鍮꾨�踰덊샇 李얘린", "log_off.searchpass" ],
		[ "�꾩�留� 蹂닿린", "log_off*k.help" ],
		[ "�쇳쉶�� 濡쒓렇��", "log_off.loginone" ]
	],
	installed : false,
	isMSIE : fc_isIE,
